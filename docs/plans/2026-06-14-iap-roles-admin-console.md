...
- [ ] **Step 1: Write the ADR** (MADR-lite) recording: the 4-role model (resolving ADR 0078 open #1), the app-never-touches-the-Group decision, "Add member = pre-assign role", the in-console request flow (no email), the default-deny middleware (opt-out) over per-route dependencies, and `dev` operator = implicit admin. Sections: `## Context`, `## Alternatives`, `## Decision`, `## Consequences`. Cross-reference ADR 0078 and the spec.
