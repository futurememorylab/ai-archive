...
Run against `backend/` (115 files):

| Metric | Value | Reference band |
|---|---|---|
| Erosion ratio | **0.373** | human median ~0.31, human upper end 0.41–0.46, agent avg ~0.68 |
| Max function CC | **25** | Radon "risky" boundary is CC 10 |
| Functions with CC > 10 | 56 | — |
...
- **Double-count guard:** the offender-flattening must yield each callable
  exactly once. The measurement spike showed some functions appearing twice
  (closure/method walk overlap). The implementation will dedupe by
  `(path, name, lineno)` and an accompanying unit test will assert a function
  is counted once so the ratio is not skewed.
...
|---|---|---|
| Erosion ratchet | baseline `0.373`, `--tolerance 0.005` | "Never get worse"; matches basedpyright/interrogate ratchets. No absolute ceiling yet. |
...
| Duplication | `pylint ... --enable=duplicate-code`, `min-similarity-lines` tuned to pass today | Binary gate; catches new large clones only. |
...
```yaml
      ...
      - id: duplicate-code
        ...
        entry: .venv/bin/pylint backend --disable=all --enable=duplicate-code --min-similarity-lines=<TUNED>
        language: system
        pass_filenames: false
        always_run: true
```
...
2. Measure and commit `.erosion-baseline.json` (0.373).
3. Tune `min-similarity-lines` to the lowest value that passes today; record it.
...
1. **Gate passes on a clean tree.** Setup: clean checkout on this branch,
   ...
   prints `Erosion: 0.373` (±) and `OK: erosion ... within baseline`.
...
3. **Rising erosion is blocked by the ratchet.** Setup: add several CC-12–15
   ...
   `0.373 + 0.005`. Action: `git commit`. Expected: rejected with
   `FAIL: erosion <n> exceeds baseline 0.373 (+0.005)`. Removing the functions
