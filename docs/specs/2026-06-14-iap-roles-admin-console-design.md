...
decisions (IAP direct on Cloud Run; the seam) already fixed in the 06-13 spec / ADR 0084.
**Decision record:** ADR 0084. This spec resolves the 06-13 spec's OPEN decisions #1 (roles
...
1. **Google owns the gate; the app owns roles only.** IAP + a Google Group/domain decides
   *who can reach the app*. The app's `user_roles` table decides *what a reached user may
   ...
   credential — that surface is deliberately not taken on). This keeps ADR 0084's non-goal
...
- `backend/app/auth/` seam: `get_current_user()` / `resolve_user()` → `CurrentUser(email,
  ...
  `tests/unit/test_auth_seam_boundary.py` + (per ADR 0084) an import-linter contract.
...
Two layers, one seam (unchanged from ADR 0084):
