...
- **Create** `docs/adr/0058-structural-erosion-gate.md` + **modify** `docs/decisions.md`.
...
### Task 6: ADR 0058 + decisions index

**Files:**
- Create: `docs/adr/0058-structural-erosion-gate.md`
...
```bash
git add docs/adr/0058-structural-erosion-gate.md docs/decisions.md
git commit -m "docs(adr): 0058 structural erosion gate"
```
...
- **Spec coverage:** erosion script (T2), dedupe/double-count guard (T2 test #3), ratchet baseline 0.410 (T3), `--max-cc 30` (T5), pylint duplication at 14 (T4/T5), `radon`+`pylint` deps (T1), ADR 0058 + decisions row (T6), all six spec "Manual acceptance flows" exercised across T3/T5 (clean pass, CC block, ratchet block, clone block via T4, re-baseline via T3 write-baseline). ✓
