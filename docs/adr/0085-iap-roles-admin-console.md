# 0085. App-side roles + admin console on top of IAP (default-deny gate, app-never-touches-the-Group)
...
> **Update (2026-06-15):** the role model below was originally a 4-role ladder
> (Admin / Annotator / Publisher / Viewer with V·P·A·M dots). It was **simplified
> to two roles — `admin` (manages access + everything) and `member` (a regular
> non-admin user: view / publish / run AI)** before shipping; the V·P·A·M
> permissions column was removed and pending requests gained an "Accept" action.
> The two-layer architecture, default-deny gate, and never-touch-the-Group
> decisions are unchanged. The sections below describe the original 4-role intent.

## Context

ADR 0084 fixed the *mechanism* (Google IAP gates the app; a small app-side roles table
...
conflicts with 0084's non-goal of managing the gate inside the app.

## Alternatives

- **Who owns "who can reach the app"?** (a) **Google owns the gate, app owns roles only**
  (chosen) — keeps 0084's non-goal intact, smallest security-critical surface; (b) app owns
...
- **Role model:** 2 roles (`admin`/`member`, 0084's strawman) vs the design's **4 roles**
