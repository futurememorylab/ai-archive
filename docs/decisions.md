...
| 0103 | 2026-06-18 | [Published timeline band is reactive (opt-in `x_for` Alpine loop, clip-detail only; studio stays static) and the view auto-switches to Published scope after a confirmed sync — so published markers turn orange without a reload instead of lingering blue in the draft band](./adr/0103-published-timeline-reactive-and-scope-auto-switch-on-publish.md) |
