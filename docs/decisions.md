...
| 0099 | 2026-06-17 | [Clip version history: publish snapshots (clip_versions, local-canonical + pragafilm.anno_version breadcrumb), re-run replaces the working draft, restore-forward, one publish-state headline (Live/Draft/Publishing/Failed); built as a snapshot layer on the existing write queue/SyncEngine](./adr/0099-clip-version-history-publish-snapshots.md) |
