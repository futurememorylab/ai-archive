...
| 0090 | 2026-06-17 | [Drop "last sign-in" tracking from the admin console (UI + backend); gate the invited→active flip behind get_gate_state so browsing is read-only (fixes #73 per-request write contention)](./adr/0090-drop-last-seen-from-admin-console.md) — *supersedes last-seen aspects of 0085* |
