...
| 0125 | 2026-06-24 | [Annotation/studio jobs run on a single lifespan-owned DB-backed claim worker (`JobRunner`), the queue-paradigm twin of `MediaPrefetcher` (Tier 2 of issue #100): routes become pure DB writers that insert a `pending` job and return; the worker polls, claims the oldest pending job via CAS, runs `run_job`, and loops. Orphaned `running` jobs are **resumed** (`requeue_orphaned_running`: running→pending + transient items reset) on worker start, not cancelled. Removes `_running_jobs` / `drain_running_jobs` / `cancel_orphaned_running` / `only_clip_ids` / `auto_start`; cancel flips DB state then `job_runner.cancel`s the in-flight job. Calibration's `force_resolution` / `record_only` move onto the `jobs` row (migration 0029) so the one worker runs sweeps identically](./adr/0125-jobrunner-claim-worker.md) — *Invariant; Tier 2 of #100, completes 0115; cross-ref 0114* |
| 0126 | 2026-06-30 | [Multi-statement DB writes hold `ctx.write_lock` (asyncio.Lock on CoreCtx) across `commit=False`→`commit()` windows so concurrent writers can't prematurely commit each other's transactions on the shared aiosqlite connection; `_handle_result` exceptions route through `_retry_or_fail` so rows never strand `in_flight`; AST guard enforces every `commit=False` call is inside a `write_lock` block](./adr/0126-write-transaction-lock-and-handle-result-hardening.md) — *Invariant; fixes #115* |
