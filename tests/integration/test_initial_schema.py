...
async def _seed_version(conn):
    ...
    await conn.execute(
        ...
        VALUES (1, 1, 1, 'production', 'p', '{}', '{}', 'gemini-2.5-pro',
...
async def test_fts5_handles_czech_diacritics(tmp_path):
    ...
    async with open_db(db) as conn:
        await apply_migrations(conn, MIGRATIONS)
        await _seed_version(conn)
        await conn.execute(
            """
            INSERT INTO annotations
              (catdv_clip_id, catdv_clip_name, prompt_version_id, model, prompt_used,
