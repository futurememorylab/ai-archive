...
def test_synced_count_counts_only_server_confirmed_applied_items():
    # applied_count = enqueued; synced_count = confirmed on CatDV (synced_at set).
    draft = {
        "has_draft": True,
        "markers": [
            {
    ...
    a = draft_review_arrays(draft)
    assert a["applied_count"] == 2  # both enqueued
    assert a["synced_count"] == 1  # only one confirmed on the server
    assert a["markers"] == []  # applied items are excluded from the panel
