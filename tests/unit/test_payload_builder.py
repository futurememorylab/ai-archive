...
def test_note_append_mode_joins_with_separator():
    item = ReviewItem(
        ...
        proposed_value="New AI annotation.",
        decision="accepted",
    )
    payload = build_put_payload(
        current=SAMPLE_CLIP,
    ...
    assert (
        payload["fields"]["pragafilm.popis.materialu"]
        == "Existing notes.\n\n---\n\nNew AI annotation."
    )
