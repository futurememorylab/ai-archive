...
async def test_run_fails_clearly_when_neither_cached(db):
    ...
    proxy = MagicMock()
    proxy.path_for_clip_id = AsyncMock(
        side_effect=ProxyNotFound("clip 42 not cached locally")
    )

    ai_store = MagicMock()
    ai_store.status = AsyncMock(return_value=None)
    ai_store.ensure_uploaded = AsyncMock(side_effect=AssertionError("ensure_uploaded called"))
    ai_store.reference_for_gemini = AsyncMock(side_effect=AssertionError("reference_for_gemini called"))
