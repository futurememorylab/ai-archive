...
(status 'invited') — it does NOT open the Google gate; a human admin must also
add them to the Google Group.

`GET /admin/access` renders the section partial (loaded into the admin console
by the "Access & Permissions" tab). CRUD endpoints return the members-table
partial on success and push a toast; never location.reload(). Self-protection +
last-admin guard are enforced server-side, not just hidden in the UI.
"""
...
async def access_section(request: Request, role: str = "", status: str = "", q: str = ""):
    require_role(request, "admin")
    data = await _members_ctx(request, role=role, status=status, query=q)
    return templates.TemplateResponse(request, "pages/_admin_access.html", data)


@router.get("/admin/access/members", response_class=HTMLResponse)
async def access_members(request: Request, role: str = "", status: str = "", q: str = ""):
    require_role(request, "admin")
    data = await _members_ctx(request, role=role, status=status, query=q)
